...
def add_device(routers, dc_uid, device_data, collectors, devices_fulllist):
...
def import_properties(routers, uid, new_data):
    properties_router = routers['Properties']
    # cProperties
...
def import_deviceclass(routers, device_classes_list, dc_data):
    device_router = routers['Device']
    ...
    # Properties
    import_properties(routers, dc_uid, new_data)
    # Devices
...
def parse_deviceclasses(routers, input):
    print('Loading input file')
    data = yaml.safe_load(file(input, 'r'))         # dict
    print('Loaded input file')
    ...
    for dc in dc_loop:
        ...
        import_deviceclass(routers, device_classes_list, dc_data)
        ...
        if dc_devices:
            ...
            for device in dev_loop:
                ...
                add_device(routers, dc_uid, device_data, collectors, devices_fulllist)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', dest='output', action='store', default='devices_input.yaml')
